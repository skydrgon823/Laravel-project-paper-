                    ...
                    @foreach ($productImages as $img)
                        <figure class="border-radius-10">
                            <a href="{{ RvMedia::getImageUrl($img) }}">
                                <img src="{{ RvMedia::getImageUrl($img, 'medium') }}" alt="{{ $product->name }}" style="width: 60%; height:auto">
                            </a>
                        </figure>
                    @endforeach
                </div>
...
fa-linkedin"></i></a>
        ...
        <div class="col-md-6 col-sm-12 col-xs-12">
            <div class="detail-info">
                @if ($product->brand->id)
                    ...
                    @endif
                <h2 class="title-detail" style="margin-bottom: 0px">{{ $product->name }}</h2>
                ...
                {{-- <div class="bt-1 border-color-1 "></div> custom --}}
                ...
                @if ($product->sku)
                <div class="mb-2"><span class="d-inline-block p-font-sw" id="product-sku">Artikelnummer</span>: <span class="p-font-sw">{{ $product->sku }}</span></div>
                <div class="mb-5"><span class="d-inline-block p-font-sw" id="product-sku">Herstellernummer</span>: <span class="p-font-sw">{{ $product->sku }}</span></div>
                @endif
                {{-- @if ($product->variations()->count() > 0)
                    ...
                    <div class="number-items-available" style="@if (!$product->isOutOfStock()) display: none; @endif margin-bottom: 10px; display:none">
                ...
                <form class="add-to-cart-form" method="POST" action="{{ route('public.cart.add-to-cart') }}" style="max-width: 100%; border-radius: 20px; background-color: #e9f0f7; margin: 10px auto;  padding: 44px 60px;">
                    @csrf
                    ...
                    <div class="detail-extralink">
                        ...
                        <div class="detail-qty border radius" style="max-width: 100%; border-radius: 20px;">
                        ...
                        <div class="product-extra-link2 @if (EcommerceHelper::isQuickBuyButtonEnabled()) has-buy-now-button @endif">
                            @if (EcommerceHelper::isCartEnabled())
                                <button type="submit" class="button button-add-to-cart @if ($product->isOutOfStock()) btn-disabled @endif" type="submit" @if ($product->isOutOfStock()) disabled @endif><i class="far fa-heart" style="padding:0 10px"></i>Zur Wunschliste hinzufügen</button>
                            ...
                            {{-- @if (EcommerceHelper::isWishlistEnabled())
                                <a aria-label="{{ __('Add To Wishlist') }}" class="action-btn hover-up js-add-to-wishlist-button"  style="width: 100%; margin-top:15px; border-radius: 20px; background-color: #12a05c;color: white;"data-url="{{ route('public.wishlist.add', $product->id) }}" href="#"><i class="far fa-heart"></i>   Zur Wunschliste hinzufügen</a> 
                            @endif --}}
    ...
    <div class="tab-style3">
        <ul class="nav nav-tabs text-uppercase">
            ...
            @endif --}}
            <li class="nav-item">
                <a class="nav-link" id="Produktangaben-tab" data-bs-toggle="tab" href="#Produktangaben">Produktangaben</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="Download-tab" data-bs-toggle="tab" href="#Download">Download</a>
            </li>
        </ul>
        <div class="tab-content shop_info_tab entry-main-content">
            ...
            <div class="tab-pane fade" id="Reviews">
                ...
                <div class="comment-form" @if (!$product->reviews_count) style="border: none" @endif>
                    <h4 class="mb-2">{{ __('Add a review') }}</h4>
                    <div class="row">
            ...
            @endif
            <div class="tab-pane fade" id="Produktangaben">
                {!! apply_filters('ecommerce_before_product_description', null, $product) !!}
                {!! BaseHelper::clean($product->description) !!}
                {!! apply_filters('ecommerce_after_product_description', null, $product) !!}
            </div>
